####################################################################################################
#
#  	--- Last updated:  06.24.2021 BY Shawn Serbin <sserbin@bnl.gov>
####################################################################################################
...
for (i in seq_along(1:2)) {
  ...
  plot(fluxes[[sites[i]]]$data.fluxCo2.nsae.flux~fluxes[[sites[i]]]$timeBgn, 
...
for (i in seq_along(1:2)) { #-- could just make this a function
  ...
  nsae_co2_plot_data <- fluxes[[sites[i]]] %>%
  ...
  png(file = file.path(figure_dir, paste0(sites[i],"_NSAE_CO2_flux_all_dates_QC.png")), 
      height=3000,width=4300, res=340)
  par(mfrow=c(1,1), mar=c(4.5,5.0,1,0.4), oma=c(0.3,0.9,0.3,0.1)) # B, L, T, R
  plot(nsae_co2_plot_data$data.fluxCo2.nsae.flux~nsae_co2_plot_data$timeBgn, 
       pch=".", xlab="Date", ylab="Net Surface-Atmosphere CO2 flux (umolCo2 m-2 s-1)", ylim=c(-26,26),
