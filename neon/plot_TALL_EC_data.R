...
term <- unlist(strsplit(names(fluxes$TALL), split=".", fixed=T))
...
plot(pr.top$PARMean~pr.top$startDateTime, pch=20, xlab="Date", 
     ylab="PAR (umols m-2 s-1)", main="NEON TALL")
box(lwd=2.2)
dev.off()
...
plot(Tair$TAAT_30min$tempTripleMean~Tair$TAAT_30min$startDateTime, pch=20, xlab="Date", 
     ylab="Tair (deg C)", main="NEON TALL")
box(lwd=2.2)
dev.off()
#--------------------------------------------------------------------------------------------------#
...
#      xlim=c(as.POSIXct("2018-07-05", tz="GMT"),
#             as.POSIXct("2018-07-07", tz="GMT")),
#      ylim=c(-24,24), xaxt="n")
# lines(fluxes[[site[site_num]]]$data.fluxCo2.nsae.flux~fluxes[[site[site_num]]]$timeBgn, 
#       xlim=c(as.POSIXct("2018-07-05", tz="GMT"),
#              as.POSIXct("2018-07-07", tz="GMT")))
# axis.POSIXct(1, x=as.POSIXct(fluxes[[site[site_num]]]$timeBgn), 
#              format="%Y-%m-%d %H:%M:%S")
# 
